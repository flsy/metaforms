...
describe('maxlength', () => {
  ...
  it('should return an error if the entered text exceeds the max length rule of the second rule', () => {
    const errorMessage = validateField(
      ...
      {
        value: 'honza',
        validation: [maxlength({ message: 'max 6 characters long', value: 6 }), maxlength({ message: 'max 4 characters long', value: 4 }), maxlength({ message: 'max 2 characters long', value: 2 })],
      },
    );
